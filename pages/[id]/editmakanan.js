...
const EditMakanan = ({ makanan }) => {
  ...
  const updateMakanan = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/makanans/${router.query.id}`, {
        method: 'PUT',
        headers: {
    ...
    <section className="text-gray-600 body-font">
      <div className="container mx-auto flex px-5 py-24 items-center justify-center flex-col">
        <div className="text-center lg:w-2/3 w-full">
          <h1 className="title-font sm:text-4xl text-3xl mb-20 font-medium text-gray-900">Edit Makanan</h1>
          <div>
            {isSubmitting ?
...
EditMakanan.getInitialProps = async ({ query: { id } }) => {
